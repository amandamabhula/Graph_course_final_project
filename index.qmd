...
# Prepare clean dataset by replacing "k" in values greater than 1000, by multiplying by 1000
...
# Convert male lung cancer dataset to long format and find country with highest and lowest new cases in 2019
...
total_new_lung_cancer_cases = int(lung_cancer_2019["new_lung_cancer_cases"].sum())
...
# Map: Choropleth map showing new lung cancer cases in 2019

lung_cancer_2019["country_code"] = coco.convert(lung_cancer_2019["country"], to="ISO3")
lung_cancer_2019

## re-map UAE and drop "Serbia and Montenegro" since these two exist as separate countries in the dataset

custom_map =  {"UAE" : "United Arab Emirates"}

lung_cancer_2019["country"] = lung_cancer_2019["country"].replace(custom_map)

lung_cancer_2019["country_code"] = coco.convert(lung_cancer_2019["country"], to="ISO3", not_found="original")

lung_cancer_2019 = lung_cancer_2019.drop(index = 982)
lung_cancer_2019
...
fig_1 = px.choropleth(
    ...
    hover_name="country",
    title="New male lung cancer cases: 2019",
...
# add continent data
lung_cancer_2019["continent"] = lung_cancer_2019["country"].apply(lambda x: coco.convert(names=x, to="continent"))

lung_cancer_2019

highest_new_cases_countries = lung_cancer_2019.loc[lung_cancer_2019.groupby("continent")["new_lung_cancer_cases"].idxmax()].sort_values(
...
`{python} str(total_new_lung_cancer_cases)`
...
co2_long["total_co2"] = pd.to_numeric(co2_long["total_co2"].astype(str).str.replace("−", "-"))

# replace UAE with United Arab Emirates in country and add continent info to the df

co2_long["country"] = co2_long["country"].replace(custom_map)
...
# Stacked bar chart:
# Creating a new categorical variable for total CO2 with Custom Functions, using arbitrary cutoffs, then analyzing by continent
...
lung_cancer_co2_merged = pd.merge(
    ...
    co2_long_2019,
    how="left",
    on=["country", "year", "continent"]).sort_values(["country", "year", "continent"]).dropna()
